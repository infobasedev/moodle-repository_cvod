...
/**
 * This plugin is used to access cvod files
 *
 * @since Moodle 2.0
...
class repository_cvod extends repository {

    public function __construct($repositoryid, $context = SYSCONTEXTID, $options = array()) {
        parent::__construct($repositoryid, $context, $options);
        $this->keyword = optional_param('cvod_keyword', '', PARAM_RAW);
        $this->licensekey = trim(get_config('cvod', 'licensekey'));
        $this->launchUrl  = trim(get_config('cvod', 'launchURl'));
    }
    ...
    public function get_listing($path = '', $page = '') {
        global $COURSE;
        $callbackurl = new moodle_url('/repository/cvod/callback.php', array('repo_id'=>$this->id));
    ...
    public static function type_config_form($mform, $classname = 'repository') {
        ...
        $licensekey = get_config('cvod', 'licensekey');
        $launchURl  = get_config('cvod', 'launchURl');
        ...
        $mform->addElement('text', 'launchURl', get_string('launchURl', 'repository_cvod'), array('value'=>$launchURl,'size' => '500'));
        $mform->setType('launchURl', PARAM_URL);
        $mform->addRule('launchURl', $strrequired, 'required', null, 'client');
        
        $mform->addElement('text', 'licensekey', get_string('licensekey', 'repository_cvod'), array('value'=>$licensekey,'size' => '500'));
